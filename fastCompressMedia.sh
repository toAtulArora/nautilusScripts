...
do gnome-terminal -x ffmpeg -i "$I" -c:v libx264 -crf 23 -preset medium -c:a ac3 -vbr 4 \
-movflags +faststart -vf scale=-2:720,format=yuv420p "${I%.*}"compressed.mp4; #ffmpeg -i "$I" -filter:v scale="$width":-1 -strict -2 "${I%.*}"compressed."$extension"; #2>&1 | zenity
